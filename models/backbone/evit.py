...
class Attention(nn.Module):
    ...
    def forward(self, x, keep_rate=None, tokens=None):
        ...
        if self.keep_rate < 1 and keep_rate < 1 or tokens is not None:  # double check the keep rate
            ...
            a, idx = torch.topk(cls_attn, left_tokens, dim=1, largest=True, sorted=True)  # [B, left_tokens]
            print('idx',idx.shape)
...
class EViT(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=4,
                 ...
                 act_layer=None, weight_init='', keep_rate=(0.9,), fuse_token=False):
        """
        Args:
    ...
    def forward_features(self, x, keep_rate=None, tokens=None, get_idx=True):
        _, _, h, w = x.shape
