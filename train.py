...
import configs.swin as s_cfg
import configs.swin_pose as p_cfg

from models.model import SwinPose
from datasets.COCO_regression import COCODataset1
...
@click.command()
@click.option('--config-path', type=click.Path(exists=True), default='config.yaml', required=False,
              help='train config file path')
@click.option('--model-name', type=str, default='s', required=True)
def main(config_path, model_name):
    cfg = {
           's': s_cfg,
           'p': p_cfg}.get(model_name.lower())
    # Load config.yaml
    ...
    linear_scaled_lr = cfg.TRAIN['BASE_LR']* cfg.data['samples_per_gpu'] / 512.0
    linear_scaled_warmup_lr = cfg.TRAIN['WARMUP_LR'] * cfg.data['samples_per_gpu'] / 512.0
    linear_scaled_min_lr = cfg.TRAIN['MIN_LR'] * cfg.data['samples_per_gpu'] / 512.0
    ...
    datasets_train = COCODataset1(
        root_path=r'D:\Pythoncoda\human3.6dataset\datasets\coco',
        data_version="val2017",
        is_train=False,
        use_gt_bboxes=True,
        image_width=cfg.data_cfg['image_size'][0],
        image_height=cfg.data_cfg['image_size'][1],
        scale=False,
        scale_factor=0,
        flip_prob=0,
        rotate_prob=0,
