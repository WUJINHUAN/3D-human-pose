...
class COCODataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        return image_transformed, target.astype(np.float32), target_weight.astype(np.float32), joints_data
...
if __name__ == '__main__':
    ...
    coco = COCODataset(root_path=f"{os.path.dirname(__file__)}/COCO", data_version="train2017", rotate_prob=0.,
                       half_body_prob=0.)
    item = coco[1]
